    ...
    */
    ...
    void SrdSearchDialog::InitDialog(HWND hwnd, LPARAM lParam)
    {
        ...
        LVCOLUMN routeStringColumn = {
            LVCF_FMT | LVCF_WIDTH | LVCF_TEXT | LVCF_SUBITEM, LVCFMT_LEFT | LVCFMT_FIXED_WIDTH, 600, routeString, 2};
    ...
    void SrdSearchDialog::StartSearch(HWND hwnd)
    {
        ...
        SendDlgItemMessage(hwnd, IDC_SRD_DESTINATION, WM_GETTEXT, 255, reinterpret_cast<LPARAM>(&destinationBuffer));
    ...
    void SrdSearchDialog::SelectSearchResult(HWND hwnd, NMLISTVIEW* details)
    {
        ...
        if (((details->uNewState ^ details->uOldState) & LVIS_SELECTED) && !this->previousSearchResults.empty()) {
            ...
            std::wstring noteStringWide =
                HelperFunctions::ConvertToWideString(this->FormatNotes(this->previousSearchResults, details->iItem));
